...
exports.signup = catchAsync(async (req, res, next) => {
	...
	try {
		...
		const url = `${req.protocol}://${req.get(
			"host"
		)}/api/v1/users/verify-email?token=${verifyEmailToken}`;
...
exports.protect = catchAsync(async (req, res, next) => {
	...
	if (!currentUser) {
		return next(
			new AppError("The user does no longer exists. Please log in again", 401)
		);
	}
...
exports.restrictTo = (...roles) => {
	return (req, res, next) => {
		if (!roles.includes(req.user.role)) {
			return next(new AppError("You have no permission to perform this task!"));
		}
